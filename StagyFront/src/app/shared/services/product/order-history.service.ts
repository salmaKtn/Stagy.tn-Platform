...
import {SortColumn, SortDirection} from '../../directive/order-history.directive';
...
function sort(orders: orderHistory[], column: SortColumn, direction: string): orderHistory[] {
  if (direction === '' || column === '') {
    return orders;
  } else {
...
function matches(order: orderHistory, term: string, pipe: PipeTransform) {
  return order.product.toLowerCase().includes(term.toLowerCase())

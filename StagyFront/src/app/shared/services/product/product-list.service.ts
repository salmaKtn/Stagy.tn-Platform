...
function sort(orderList: productList[], column: SortColumn, direction: string): productList[] {
  if (direction === '' || column === '') {
    return orderList;
  } else {
...
export class ProductListService {
  ...
  get orderList$() { return this._orderList$.asObservable(); }
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 2. filter
    orderList = orderList.filter(ticket => matches(ticket, searchTerm, this.pipe));
